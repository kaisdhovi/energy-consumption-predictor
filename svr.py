...
def process_outliers(df, aggregation_cols):
    ...
    numerical_cols_for_outliers = ['Total_Energy_kWh', 'Global_reactive_power', 
                                   'Global_intensity', 'Voltage', 'Sub_metering_1_kWh', 
...
def train_and_evaluate_model(X_train_scaled, X_test_scaled, y_train, y_test, X_test_index, start_date, end_date):
    ...
    results_df = pd.DataFrame({"Actual": y_test, "Predicted": y_pred}, index=X_test_index)
...
if dataset_file is not None:
    with st.spinner("Memproses data..."):
        ...
        if energy_df_indexed is not None:
            ...
            # 2. Agregasi per jam
            energy_hourly_summary, aggregation_cols = aggregate(energy_df_indexed)
            ...
            st.subheader("Data Setelah Fitur Waktu")
